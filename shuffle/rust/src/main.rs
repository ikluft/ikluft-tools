...
fn path_from_cli() -> Result<PathBuf, anyhow::Error> {
    // get input file name from command line
    let args: Vec<String> = args().collect();
    if args.len() < 2 {
        bail!("file name parameter missing");
    }
    let infile_path = Path::new(&args[1]).to_owned();
...
fn read_file_lines(infile_path: &PathBuf) -> Result<Vec<String>, anyhow::Error> {
